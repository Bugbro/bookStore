...
export const getAllOrders = async (req, res) => {
    try {
        ...
        .sort({ createdAt: -1});
        if(!orders || orders.length === 0) return resHandler(res, 404, "No orders found.");
...
export const getMonthlyRevenue = async (req, res) => {
    try {
        const revenue = await Order.aggregate([
            //stage 1: filter orders with status "Delivered"
            {
            ...
            //stage 2 group by month and year
            {
                $group:{
                    ...
                    //add total revenue amount
                    totalRevenue: { $sum: "$totalAmount" },
                    totalOrders: { $sum: 1}
                }
            }
        ]);
        if(!revenue) return resHandler(res, 404, "No revenue data found.");
        return resHandler(res, 200, "Monthly revenue calculated", revenue);
    } catch (error) {
        console.log("Error while getting monthly revenue", error.message);
        return resHandler(res, 500, error.message);
    }
};
...
export const getYearlyRevenue = async(req, res)=>{
    try {
        ...
        if(!revenue || revenue.length === 0) return resHandler(res, 404, "No revenue data found for the year.");
        return resHandler(res, 200, "Total year revenue calculated", revenue);
    } catch (error) {
        console.log("Error while getting yearly revenue", error.message);
        return resHandler(res, 500, error.message);
    }
};